...
import com.github.krrrr38.mackerel4s.model.{ SuccessResponse, ServiceMetric }

object PostServiceMetric extends App {
  if (args.length < 3) {
    println(s"[${Console.RED}error${Console.RESET}] apikey could not be found")
    println(s"""[${Console.RED}error${Console.RESET}] usage: `sbt "mackerel-client-scala-example/runMain com.github.krrrr38.mackerel4s.example.PostServiceMetric apikey servicename metricname"`""")
  } else {
    val apikey = args(0)
