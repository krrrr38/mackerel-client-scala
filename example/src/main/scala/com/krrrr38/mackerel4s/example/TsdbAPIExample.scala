...
import com.krrrr38.mackerel4s.model.{ LatestTsdbResponse, SuccessResponse, HostMetric }

object PostTsdb extends App {
  if (args.length < 3) {
    println(s"[${Console.RED}error${Console.RESET}] apikey could not be found")
    println(s"""[${Console.RED}error${Console.RESET}] usage: `sbt "mackerel-client-scala-example/runMain com.krrrr38.mackerel4s.example.PostTsdb apikey hostid metricname"`""")
  } else {
    val apikey = args(0)
    val hostId = args(1)
...
object LatestTsdb extends App {
  if (args.length < 2) {
    println(s"[${Console.RED}error${Console.RESET}] apikey could not be found")
    println(s"""[${Console.RED}error${Console.RESET}] usage: `sbt "mackerel-client-scala-example/runMain com.krrrr38.mackerel4s.example.LatestTsdb apikey hostid"`""")
  } else {
    val apikey = args(0)
    val hostId = args(1)
    val mackerel = new Mackerel(apikey)
